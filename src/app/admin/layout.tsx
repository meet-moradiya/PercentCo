...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background flex transition-colors duration-300">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 bottom-0 w-64 bg-surface border-r border-surface-border flex flex-col shrink-0 transition-all duration-300 z-50 lg:sticky lg:top-0 lg:h-screen ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        {/* Brand */}
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 text-sm tracking-wide transition-all duration-200 ${
                  active ? "bg-gold/10 text-gold border-l-2 border-gold" : "text-muted hover:text-foreground hover:bg-surface-light"
                }`}
              >
                {item.icon}
                {item.label}
        ...
        <div className="p-4 border-t border-surface-border space-y-2">
          ...
          <Link href="/" className="flex items-center gap-3 px-4 py-2.5 text-sm text-muted hover:text-foreground transition-colors">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
      ...
      <main className="flex-1 min-w-0">
        ...
        <div className="lg:hidden sticky top-0 z-30 bg-surface border-b border-surface-border px-4 py-3 flex items-center justify-between">
          ...
          <h1 className="text-gold text-sm tracking-widest uppercase" style={{ fontFamily: "'Playfair Display', serif" }}>
            PERCENTCO
          </h1>
          <div className="w-6" />
        </div>
