...
export default function AdminReservations() {
  ...
  const openAssignModal = async (reservation: Reservation) => {
    ...
    try {
      const res = await fetch(
        `/api/availability?date=${reservation.date}&time=${reservation.time}&guests=${reservation.guests}`
      );
      const data = await res.json();
  ...
  const statusColor = (status: string) => {
    switch (status) {
      ...
      case "seated": return "text-blue-400 bg-blue-400/10 border-blue-400/30";
      case "completed": return "text-muted bg-muted/10 border-muted/30";
      case "no-show": return "text-orange-400 bg-orange-400/10 border-orange-400/30";
  ...
  const displayed = timeFilter
    ? reservations.filter((r: Reservation) => r.time === timeFilter)
    : reservations;
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6">
        ...
        {(dateFilter || timeFilter) && (
          <button onClick={() => { setDateFilter(""); setTimeFilter(""); setPage(1); }} className="px-2 py-1.5 text-red-400 text-xs hover:text-red-300">
            Clear
          </button>
        )}
      </div>
              ...
              {displayed.map((r: Reservation) => (
                <tr key={r._id} className="hover:bg-surface-light transition-colors">
                  <td className="px-3 py-3">
                    ...
                    {r.occasion !== "None" && (
                      <p className="text-gold text-[10px] mt-0.5">{r.occasion}</p>
                    )}
                  </td>
                  <td className="px-3 py-3">
                  ...
                  <td className="px-3 py-3">
                    <span className={`px-2 py-0.5 text-[10px] uppercase tracking-wider border ${statusColor(r.status)}`}>
                      {r.status}
                    </span>
                  </td>
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap gap-1">
                      {r.status === "pending" && (
                        <button onClick={() => openAssignModal(r)} className="px-2 py-0.5 text-[10px] text-green-400 border border-green-400/30 hover:bg-green-400/10 transition-colors">Confirm</button>
                      )}
                      {r.status === "confirmed" && (
                        <button onClick={() => updateStatus(r._id, "seated")} className="px-2 py-0.5 text-[10px] text-blue-400 border border-blue-400/30 hover:bg-blue-400/10 transition-colors">Seat</button>
                      )}
                      {r.status === "seated" && (
                        <button onClick={() => updateStatus(r._id, "completed")} className="px-2 py-0.5 text-[10px] text-muted border border-muted/30 hover:bg-muted/10 transition-colors">Complete</button>
                      )}
                      {r.status === "confirmed" && (
                        <button onClick={() => updateStatus(r._id, "no-show")} className="px-2 py-0.5 text-[10px] text-orange-400 border border-orange-400/30 hover:bg-orange-400/10 transition-colors">No-Show</button>
                      )}
                      {["pending", "confirmed"].includes(r.status) && (
                        <button onClick={() => updateStatus(r._id, "cancelled")} className="px-2 py-0.5 text-[10px] text-red-400 border border-red-400/30 hover:bg-red-400/10 transition-colors">Cancel</button>
                      )}
                      <button onClick={() => deleteReservation(r._id)} className="px-2 py-0.5 text-[10px] text-red-400/60 border border-red-400/20 hover:bg-red-400/10 transition-colors"><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-surface border border-surface-border w-full max-w-md">
            <div className="px-6 py-4 border-b border-surface-border flex items-center justify-between">
              ...
              <button onClick={() => setAssignModal(null)} className="text-muted hover:text-foreground text-xl">×</button>
            </div>
            <div className="p-6">
              <p className="text-muted text-sm mb-4">
                Select a table for <span className="text-foreground">{assignModal.guests} guests</span> on{" "}
                <span className="text-foreground">{assignModal.date}</span> at{" "}
                <span className="text-foreground">{assignModal.time}</span>
              </p>
